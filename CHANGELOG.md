...
- Added admin authentication and real product verification to `src/api/MagentoApiClient.ts`: `MagentoApi.authenticate()` resolves an admin bearer token once per run (prefers `MAGENTO_ADMIN_TOKEN`, else mints one via `POST /rest/V1/integration/admin/token` from `MAGENTO_ADMIN_USERNAME`/`MAGENTO_ADMIN_PASSWORD`), and `verifyProductIsAvailable(name, price)` queries the REST catalogue API (`/products` filtered by name) and asserts HTTP 200, `total_count > 0`, the matched name, and the price (backlog Item #3)
- Added `docs/admin-api-token-guide.md` — a pedagogical guide to Magento admin API tokens: the bearer-vs-integration token types, the mandatory 2.4.x admin-2FA gotcha and the test-target fix, mint/use commands (PowerShell + curl), the verified product lookup, the #3 wiring plan, and a security checklist (backlog Item #3)

### Changed

- Changed `src/step-definitions/background.steps.ts` — the `a product "…" priced at "…" is available` step is now **API-driven**: it calls `MagentoApi.verifyProductIsAvailable` instead of navigating the storefront UI, realising the "API setup, UI assertion" pattern (ADR-0003). The UI fallback (`Navigate`/`StorefrontPage`) was removed (backlog Item #3)
- Changed `src/hooks/browser.hooks.ts` — resolve the admin token once in `BeforeAll` (`MagentoApi.authenticate()`) and grant the actor the `CallAnApi` ability alongside `BrowseTheWebWithPlaywright`, so API and UI abilities coexist on the same actor (backlog Item #3)
- Changed `docs/adr/0003-api-driven-test-data-setup.md` — marked Implemented; replaced the stub/UI-fallback "Concrete detail" with the real wiring: the camelCase `searchCriteria[filterGroups]`/`conditionType` query, the bearer-token auth flow, the response shape, and the four assertions, verified against the Docker store (backlog Item #3)
- Changed `README.md` — added an Environment variables table documenting `MAGENTO_ADMIN_TOKEN` / `MAGENTO_ADMIN_USERNAME` / `MAGENTO_ADMIN_PASSWORD` (and `BASE_URL` / `HEADLESS`), and the admin-2FA caveat (backlog Item #3)
- Changed `docs/docker-magento-setup.md` — added step 6c (disable the two admin-2FA modules on the test target) so the REST admin-token endpoint works for API-driven setup; flagged test-target-only with a reversal command (backlog Item #3)
- Changed `docker-compose.yml` — raised the OpenSearch healthcheck `start_period` from `5s` to `60s`. OpenSearch takes ~50s to boot, longer than `retries × interval` (6 × 5s = 30s), so a cold `compose up` aborted with "opensearch is unhealthy" and `phpfpm`/`app` never started, forcing a second `compose up`. A single cold bring-up now succeeds to all-healthy + storefront HTTP 200 (validated 2026-06-06; smoke 7/7, `@placesOrder` 4/4 unaffected)
